...
router.post("/send", (req, res) => {
    ...
    db.none(insert, [chatId, message, email])
        .then(() => {
            ...
            db.manyOrNone('SELECT M.memberid, F.token FROM FCM_Token F, Members M, Chatmembers C WHERE M.memberid = F.memberid AND C.memberid = M.memberid AND C.chatid = $1 AND M.email <> $2', [chatId, email])
                .then(rows => {
